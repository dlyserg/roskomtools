...
def parse_registry(filename, database, register):
	...
	for item in records:
		try:
                        if register == "SR":
                                ...
                                content_urgency_type = None
                                content_entry_type = None
			...
			content_hash = item.get('hash', default = None)
                                data = (content_id, content_block_type, content_include_time, content_urgency_type, content_entry_type, content_hash, content_ts, content_decision_date, content_decision_number, content_decision_org, register)
                                cursor.execute("INSERT INTO content (content_id, content_block_type, content_include_time, content_urgency_type, content_entry_type, content_hash, content_ts, content_decision_date, content_decision_number, content_decision_org, register) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", data)
                                for domain in item.xpath('domain'):
                                        cursor.execute("INSERT INTO domains (domain_content_id, domain_text, domain_ts, register) VALUES (?, ?, ?, ?)", (content_id, domain.text, domain.get('ts', default = 0), register))
                                for subnet in item.xpath('ipSubnet'):
                                        cursor.execute("INSERT INTO subnets (subnet_content_id, subnet_text, subnet_ts, register) VALUES (?, ?, ?, ?)", (content_id, subnet.text, subnet.get('ts', default = 0), register))
                                for subnet in item.xpath('ipv6Subnet'):
                                        cursor.execute("INSERT INTO subnetsv6 (subnet_content_id, subnet_text, subnet_ts, register) VALUES (?, ?, ?, ?)", (content_id, subnet.text, subnet.get('ts', default = 0), register))
                        else:
                                content_id = str(item.get('id', default = '0'))
                                content_block_type = item.get('blockType', default = 'default')
                                content_ts = parse_ts(item.get('ts', default = '0'))
                                ...
                                if content_block_type == 'default':
                                        ...
                                        for ip in item.xpath('ip'):
                                                cursor.execute("INSERT INTO ips (ip_content_id, ip_text, ip_ts, register) VALUES (?, ?, ?, ?)", (content_id, ip.text, ip.get('ts', 0), register))
                                        for subnet in item.xpath('ipSubnet'):
                                                cursor.execute("INSERT INTO subnets (subnet_content_id, subnet_text, subnet_ts, register) VALUES (?, ?, ?, ?)", (content_id, subnet.text, subnet.get('ts', default = 0), register))
                                        for ip in item.xpath('ipv6'):
                                                cursor.execute("INSERT INTO ipsv6 (ip_content_id, ip_text, ip_ts, register) VALUES (?, ?, ?, ?)", (content_id, ip.text, ip.get('ts', 0), register))
                                        for subnet in item.xpath('ipv6Subnet'):
                                                cursor.execute("INSERT INTO subnetsv6 (subnet_content_id, subnet_text, subnet_ts, register) VALUES (?, ?, ?, ?)", (content_id, subnet.text, subnet.get('ts', default = 0), register))
